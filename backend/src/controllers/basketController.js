import { addToBasketService, getBasketItemsService, updateBasketItemStatusService, deleteBasketItemService } from "../models/basketService.js";
...
export const updateBasketItemStatus = async (req, res) => {
  const { itemName } = req.params;;
  try {
    const updatedItem = await updateBasketItemStatusService(itemName);
    res.json(updatedItem);
  } catch (err) {
    console.error(err);
    if(err.statusCode){
    ...
    res.status(500).json({ error: "Failed to update item status in basket" });
  }
};